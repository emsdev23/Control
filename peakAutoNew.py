...
recipient = ['ems@respark.iitm.ac.in','harisankarm@tenet.res.in','ritvika@respark.iitm.ac.in','tavishi@respark.iitm.ac.in']


def send_mail(peak_limit,curtime):
...
def send_mail_low(peak_limit,curtime):
    message = MIMEMultipart('alternative')
    message['Subject'] = f'EMS ALERT -  Peak Demand {peak_limit} LTO Battery OFF'
    message['From'] = sender
    message['To'] = ', '.join(recipient)
                    ...
                    if batsts[0][0] != 'OFF':
                        ...
                        def checkBatStsDC():
                            ...
                            try:
                                if convByteON == 28:
                                    print("Discharge OFF completed")
                                    send_mail_low(peak_low,curtime)
                                    time.sleep(50)
                                    logger.info("Discharge OFF completed")
                                else:
                                    checkBatStsDC()
